...
// ── Typing indicator (in-memory, merged from typing.js) ──
...
async function handleTyping(req, res) {
  ...
  if (method === 'POST') {
    ...
    if (isTyping) {
      ...
      typingState.get(chatId).set(userId, { name: userName, expiresAt: Date.now() + TYPING_TTL_MS });
    } else {
      const chatMap = typingState.get(chatId);
      if (chatMap) { chatMap.delete(userId); if (chatMap.size === 0) typingState.delete(chatId); }
    }
  ...
  if (method === 'GET') {
    ...
    if (chatMap) {
      for (const [uid, entry] of chatMap) {
        if (uid !== userId) typers.push({ id: uid, name: entry.name });
      }
    }
